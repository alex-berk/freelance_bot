...
def bot_listener():
	def setup_keys(chat_id):
		...
		if current_keys and bot.setup_step.get(chat_id) != 'setup_keys_replace':
			bot.setup_step[chat_id] = {'name': 'setup_keys'}
			...
			bot.setup_step[chat_id] = {'name': 'setup_keys_init'}
	...
	def handle_commands(message):
		...
		if bot.verify_command(message.text, 'status'):
			...
			bot.setup_step[message.chat.id] = {'name': 'stop_tacking'}
	...
	def handle_text(message):
		...
		elif bot.setup_step.get(message.chat.id, {'name': None})['name'] == 'setup_keys_init' or bot.setup_step.get(message.chat.id, {'name': None})['name'] == 'setup_keys_replace':
		...
		elif bot.setup_step.get(message.chat.id, {'name': None})['name'] == 'setup_keys_delete':
			msg_words = parse_string(message.text.lower(), ',')
			if msg_words == ['готово']:
				db_handler.update_user_keys(message.chat.id, bot.setup_step[message.chat.id]['working_keys'])
				confirm_keys_setup(message.chat.id, bot.setup_step[message.chat.id]['working_keys'])
				bot.setup_step[message.chat.id] = None
			else:
				for word in msg_words:
					try:
						bot.setup_step[message.chat.id]['working_keys'].remove(word)
						bot.send_message(f'Удалил <b>{word}</b>\nКогда закончите, нажмите "Готово" для того чтобы применить изменения.' , message.chat.id, keyboard=['✅ Готово', '❌ Отмена'] + bot.setup_step[message.chat.id]['working_keys'])
					except ValueError:
						bot.send_message(f'Не нашёл слова <b>{word}</b>' , message.chat.id, keyboard=['✅ Готово', '❌ Отмена'] + bot.setup_step[message.chat.id]['working_keys'])

		elif bot.setup_step.get(message.chat.id, {'name': None})['name'] == 'stop_tacking':
