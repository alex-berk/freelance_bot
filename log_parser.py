...
def get_new_tasks_q(log):
	new_tasks = []
	new_task_lines = [line.event[10:] for line in log if line.event.startswith('New tasks') and line.event != 'New tasks []']
...
def get_sent_tasks_q(log):
	sent_tasks = []
	sent_task_lines = [line.event.split('\'')[1] for line in log if line.event.startswith('Found task')]
	tasks = Counter()
